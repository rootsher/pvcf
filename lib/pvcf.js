...
define('pvcf', ['when', 'node', 'callbacks'], function pvcf(when, nodefn, callbacks) {
     ...
     */
    TabManager.prototype.setStartView = function setStartView(startView) {
     ...
     */
    TabManager.prototype.getStartView = function getStartView() {
     ...
     */
    TabManager.prototype.setErrorView = function setErrorView(errorView) {
     ...
     * @returns {ViewDefinition}
     */
    TabManager.prototype.getErrorView = function getErrorView() {
     ...
     */
    Navigation.prototype._runNavigationEventHandlers = function _runNavigationEventHandlers(path, options) {
        this._navigationEventHandlers.forEach(function (eventHandler) {
            eventHandler(path, options);
        });
    };
     ...
     * @method
     */
    Navigation.prototype.start = function start() {
        var self = this;

        var path = window.location.hash;
        var options = {};

        this._currentHash = path;

        self._runNavigationEventHandlers(path, options);

        window.addEventListener('hashchange', function () {
            var newPath = window.location.hash;

            if (self._currentHash !== newPath) {
                self._runNavigationEventHandlers(newPath, {});
                self._currentHash = newPath;
            }
        });
    };
     ...
     */
    Navigation.prototype.addNavigationEventHandler = function addNavigationEventHandler(eventHandler) {
        this._navigationEventHandlers.push(eventHandler);
    };
     ...
     * @param {Object} options
     */
    Navigation.prototype.go = function go(path, options) {
        window.location.hash = path;
        this._runNavigationEventHandlers(path, options);
    };
     ...
     * @param {Object} options
     */
    Navigation.handleNavigationEvent = function handleNavigationEvent(patternManager, tabManager, path, options) {
        ...
        if (hash) {
            ...
            if (patternData) {
                ...
                view = tabManager.getErrorView();
            }
        } else {
            view = tabManager.getStartView();
        }
